...
SUMMARY=${intermediateDir}/${filePrefix}_unaligned_tagged_Cellular.bam_summary.txt \
BASE_RANGE=8-17 \
BASE_QUALITY=10 \
BARCODED_READ=1 \
DISCARD_READ=false \
...
#tag_pools="${EBROOTDROPMINSEQ_TOOLS}/TagBamWithReadSequenceExtended SUMMARY=${intermediateDir}/unaligned_tagged_Pool.bam_summary_${filePrefix}.txt \
...
SUMMARY=${intermediateDir}/${filePrefix}_unaligned_tagged_Molecular.bam_summary.txt \
BASE_RANGE=1-7 \
BASE_QUALITY=10 \
BARCODED_READ=1 \
DISCARD_READ=true \
...
OUTPUT_SUMMARY=${intermediateDir}/${filePrefix}_adapter_trimming_report.txt \
...
OUTPUT_SUMMARY=${intermediateDir}/${filePrefix}_polyA_trimming_report.txt \
MISMATCHES=0 \
NUM_BASES=6 \
TRIM_TAG=XA \
